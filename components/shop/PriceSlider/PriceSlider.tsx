...
import type {RangeFilter, RangeValue} from "./price-range-types";
...
const PriceSliderPip: React.FC<PriceSliderPipProps> = ({animation = "height", isInRange}) => {
  ...
  const pip = React.useMemo(() => {
    if (animation === "height") {
      return (
        <span
          className="relative h-12 w-1 rounded-full bg-white after:absolute after:bottom-0 after:h-0 after:w-full after:rounded-full after:bg-primary after:transition-all after:!duration-500 after:content-[''] data-[in-range=true]:after:h-full"
    ...
    return (
      <span
        className="h-12 w-1 rounded-full bg-purple-500 transition-all !duration-500"
        style={{
    ...
    return (
      <div className={cn("flex flex-col gap-3", className)}>
        ...
        <div className="flex items-center">
          ...
          />
          <Divider className="mx-2 w-2 bg-white " />
          <Input
            classNames={{
              input: [
                'bg-transparent',
